...
describe("Error Handling", () => {
  ...
  describe("Validation errors", () => {
    it("should collect all validation errors from fixture", () => {
      ...
      const ageErrors = result.errors.filter(e => e.message.includes("Age must be"));
      const emailErrors = result.errors.filter(e => e.message.includes("Invalid email"));
      const salaryErrors = result.errors.filter(e => e.message.includes("Salary must be"));
    ...
    it("should validate phone numbers", () => {
      const parser = new Parser()
        ...
        .col("Phone", "phone", {
          validate: (v) => {
            ...
            return phoneRegex.test(v) ? undefined : "Phone must be in format (XXX) XXX-XXXX";
          }
        });
      ...
      expect(result.errors[0].message).toBe("Phone must be in format (XXX) XXX-XXXX");
    });

    it("should validate custom business rules", () => {
      const parser = new Parser()
        .col("Employee ID", "employeeId", {
          validate: (v) => /^E\d{3}$/.test(v) 
            ? undefined 
            : "Employee ID must match pattern E###"
        })
        .col("Department", "department", {
          validate: (v) => ["Engineering", "Marketing", "HR", "Sales"].includes(v)
            ? undefined
            : "Invalid department"
        })
        .col("Level", "level", {
      ...
      const csv = "Employee ID,Department,Level\nE001,Engineering,5\nINVALID,Marketing,3\nE002,InvalidDept,2\nE003,HR,15";
      const result = parser.parse(csv);

      expect(result.success).toHaveLength(1); // Only first row
      expect(result.errors).toHaveLength(3);
      
      expect(result.errors[0].message).toBe("Employee ID must match pattern E###");
    ...
    it("should validate ranges and constraints", () => {
      const parser = new Parser()
        .col("Score", "score", {
          transform: (v) => parseFloat(v),
          validate: (v) => v >= 0 && v <= 100 ? undefined : "Score must be between 0 and 100"
        })
        .col("Grade", "grade", {
          validate: (v) => ["A", "B", "C", "D", "F"].includes(v) 
            ? undefined 
  ...
  describe("Missing column errors", () => {
    ...
    it("should handle multiple missing columns", () => {
      ...
      const missingColumns = new Set(result.errors.map(e => e.column));
    ...
    it("should not report errors for missing nullable columns", () => {
      ...
      expect(result.success[0]).toEqual({ name: "John", age: null, email: null });
      expect(result.success[1]).toEqual({ name: "Jane", age: null, email: null });
    });
    ...
    it("should not report errors for missing columns with default values", () => {
      ...
      expect(result.success[0]).toEqual({ name: "John", age: 0, status: "active" });
    });
  });
  ...
  describe("Error reporting details", () => {
    ...
    it("should distinguish between different error types", () => {
      ...
      const validationError = result.errors.find(e => e.type === "validation");
  ...
  describe("Error recovery and continuation", () => {
    it("should continue processing after errors", () => {
      ...
      expect(result.success.map(r => r.name)).toEqual(["John", "Bob", "Charlie"]);
    });
    ...
    it("should process all rows even with early errors", () => {
      const parser = new Parser()
        .col("ID", "id", { 
          validate: (v) => v.startsWith("ID") ? undefined : "ID must start with 'ID'"
        })
        .col("Value", "value", {
          transform: (v) => parseInt(v)
        });
  ...
  describe("Complex error scenarios", () => {
    it("should handle errors in complex nested data", () => {
      ...
      const csv = 'Name,Settings\nJohn,"{""theme"": ""dark""}"\nJane,"{""color"": ""blue""}"\nBob,"invalid-json"';
      const result = parser.parse(csv);

      expect(result.success).toHaveLength(1);
      expect(result.errors).toHaveLength(2);
      
      expect(result.errors[0].message).toBe("Settings must include theme property");
    ...
    it("should handle cascading validation errors", () => {
      const parser = new Parser()
        .col("StartDate", "startDate", {
          ...
          validate: (v) => !isNaN(v.getTime()) ? undefined : "Invalid start date"
        })
        .col("EndDate", "endDate", {
          transform: (v) => new Date(v),
          validate: (v) => !isNaN(v.getTime()) ? undefined : "Invalid end date"
        })
